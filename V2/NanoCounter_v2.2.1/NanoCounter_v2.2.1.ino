...
#include <DFRobot_LCD.h> // Library for LCD
...
void brightness(unsigned char color){
  int readValue = analogRead(A0);
  int toset = readValue / 4;
  lcd.setPWM(color, toset);
...
//  Serial.begin(9600);
  ...
  pinMode(10, INPUT);  //digital pin. DO NOT GET MIXED UP
  pinMode(A1, INPUT);
  pinMode(A2, INPUT);
  pinMode(A3, INPUT);
  pinMode(A6, INPUT);
...
void loop() {
  button1 = digitalRead(10);  //not an analog pin to make room for the pot
  button2 = digitalRead(A1);
  button3 = digitalRead(A2);
  button4 = digitalRead(A3);
  button5 = digitalRead(A6);
  button6 = digitalRead(A7);
...
*/
  ...
  if(button1 == HIGH){
    if(buttonreset == 1){
      ...
      delay(100);
      noTone(8);
      while(digitalRead(10) == HIGH) {}  //not an analog pin to make room for the pot
      }
